...
1. Create a tarball of the global database directory. For example, users of the snap can use the following:

       sudo tar -czPf /recovery_db.tar.gz /var/snap/lxd/common/lxd/database/global

1. Copy the tarball to all other cluster members and replace the out-of-date database dir with the contents of the tarball. For snap users, this is:

       sudo mv /var/snap/lxd/common/lxd/database/global /global_bkup
       sudo tar -xPf /recovery_db.tar.gz

Once the tarball has been unpacked on all remaining cluster members, start the LXD daemon on all members again.
