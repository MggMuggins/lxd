...
: The most common type of disk device is a storage volume.
  Specify the storage volume name as the {config:option}`device-disk-device-conf:source source` to add a storage volume as a disk device. `virtual-machine' storage volumes (and their snapshots) can also be attached as disk devices.
